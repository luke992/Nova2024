...
def example_chat(model_name: str, stream: bool = False):
    """ 
    Examples of chat completions from the proxy
    """
    client = OpenAI(
        api_key="sk-t294DQLct5JulSvcXBgTAA", # set this!!!
    ...
    response = client.chat.completions.create(
        ...
        messages = [
            {
                "role": "user",
                "content": "Give me a api get request example?"
            }
        ],
        stream=stream
    )
    ...
    print(response.choices[0].message.content) #message from the AI
    #print(response[1][1])
